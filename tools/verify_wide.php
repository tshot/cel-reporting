...
// ---- 1. index the wide file: record_id => [column => value] ----
...
$checked = $matched = $missingCol = $mismatch = 0;
$seenIds = [];
$examples = [];
$longRows = 0;
...
while (($r = fgetcsv($lh, 0, ',', '"', ESC)) !== false) {
    ...
    if ($sample > 0) {
        if (!isset($seenIds[$id]) && count($seenIds) >= $sample) continue;
    }
    $seenIds[$id] = true;
